...
from zmq_message_passing import ZMQClient as client
...
def create_points_at(startpoint, endpoint, distance, geom, fid, force, divide):
    ...
    """ List of all point features along the lines"""
    feats = []
...
def points_along_line(layerout,
    ...
    for feature in get_features():
        ...
        fid = feature.id()
        if not geom:
            QgsMessageLog.logMessage("No geometry", "WorldEngine")
            continue
        ...
        for point in features:
            ...
            QgsMessageLog.logMessage(str(x), 'X')           
            QgsMessageLog.logMessage(str(y), 'Y')  
        try:       
            messenger.send(trajectory)
        except:
            pass
